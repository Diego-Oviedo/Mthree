...
import com.springbootTest.app.data.ToDoDao;
...
public class ToDoController {
    ...
    public ResponseEntity update(@PathVariable int id, @RequestBody ToDo todo) {
        ...
        if (id != todo.getId()) {
            ...
            response = new ResponseEntity(HttpStatus.NO_CONTENT);//Type-less ResponseEntity
        }
        return response;
    }
    ...
    public ResponseEntity delete(@PathVariable int id) {
        if (dao.deleteById(id)) {
            return new ResponseEntity(HttpStatus.NO_CONTENT);//Type-less ResponseEntity
        }
