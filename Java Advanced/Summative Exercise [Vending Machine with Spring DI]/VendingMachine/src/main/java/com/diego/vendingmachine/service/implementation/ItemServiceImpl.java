...
public class ItemServiceImpl implements ItemService {
	...
	public List<Item> createItem(Item item, Inventory inventory, int num_units) throws vendingMachinePersistenceException,
																						DataSourceException {
		...
		if(inventory.getInventory().containsKey(item.getSKU())) {//if the item already exists
			...
			audit_dao.writeAuditEntry("UPDATE_item [" + item_created.get(0).getSKU() 
													+ " - " + item_created.get(0).getItem_description() 
													+ "] {quantity updated}");
		}else {
	...
	public List<Item> deleteItem(Inventory inventory, String SKU) throws vendingMachinePersistenceException, 
																		 DataSourceException {
		...
		audit_dao.writeAuditEntry("DELETE_item [" + item_deleted.get(0).getSKU() 
													+ " - " + item_deleted.get(0).getItem_description() 
													+ "] {item deleted}");
