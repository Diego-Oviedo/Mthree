...
public class AuditDAOImpl implements AuditDAO {
	...
	public void writeAuditEntry(String entry) throws vendingMachinePersistenceException, DataSourceException {
		...
		try {
			out = new PrintWriter(new FileWriter(path + timestamp.getYear() + "-" + timestamp.getMonth() + "_"
					+ timestamp.getHour() + "H" + timestamp.getMinute() + "M_" + AUDIT_FILE, true));
		} catch (Exception e) {
			if (!e.equals(IOException.class)) {
				throw new DataSourceException("Issue when writting the audit [ERROR WITH FILE]", e.getCause());
			}
			throw new vendingMachinePersistenceException("Issue when writting the audit [ERROR PERSISTING OBJECT]",
					e.getCause());
		}
